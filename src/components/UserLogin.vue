        ...
        <div class="user-login-form">
          <div class="user-login-avatar">
            <img
              ...
              @mousedown.stop
              class="login-avatar"
              @click.stop="toggleAvatarSelect(false)"
            />
            ...
            <button
              ...
              type="button"
              @click.stop="toggleAvatarSelect(true)"
            >
              注册
            </button>
            <transition name="slide">
              <div
                class="select-avatar-warp"
                @mousedown.stop
                v-show="isRegitsterShow"
              >
                <div class="avatar-input-warp">
                  <input
                    type="text"
                ...
                <button
                  class="form-btn-register registerBtn"
                  @click.stop="registerQQ"
                  type="button"
                >
                  注册
                </button>
...
export default {
  ...
  created() {
    ...
    list = list.map((item) => {
      return (
        window.location.origin + "/" + BASE_URL + "static/img/avatar/" + item
      );
    });
  ...
  methods: {
    toggleAvatarSelect(isRegist) {
      if (isRegist) {
        if (this.isRegitsterShow) {
          this.isRegitsterShow = false;
          document.removeEventListener("click", this.hideAvatarSelect);
        } else {
          this.isRegitsterShow = true;
          this.isShow = false;
    ...
    login() {
      ...
      //  var conn="SELECT * FROM USER where username='"+this.qq+"' and password='"+this.password+"'";
      ...
      axios
        ...
        .then((response) => {
          if (response.data.code == "200") {
            ...
            console.log("返回：" + response.data.message);
            Message.error("返回：" + response.data.message);
          }
        })
        .catch((error) => {
          console.log(error);
    ...
    asLogin() {
      axios
        .post("/login", { username: this.qq, password: this.password })
        .then((response) => {
          if (response.data.code == "200") {
            console.log("登陆成功！");
