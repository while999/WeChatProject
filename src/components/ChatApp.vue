<template>
  <div class="chat-app-warp">
    ...
    ></UserLogin>
    <div
      class="app-main-panel ui-clear"
      v-drag
      v-if="loginUser.id && deviceType === 'pc'"
    >
              ...
              <template slot="info">
                ...
                <span v-if="curSession.type === 'user'" class="app-use-extInfo">
                  <i
                    class="iconfont"
                    :class="
                      curSession.deviceType === 'pc' ? 'icon-pc' : 'icon-phone'
                    "
                  ></i>
              ...
              <div class="app-card-container">
                ...
                <div class="app-card-infoRow">
                  <i
                    class="iconfont"
                    :class="
                      loginUser.deviceType === 'pc' ? 'icon-pc' : 'icon-phone'
                    "
                  ></i>
                  <span class="app-login-ip">{{ loginUser.ip }}</span>
                </div>
                <div class="app-card-infoRow">
                  <span class="app-login-time">{{
                    loginUser.time | formatTime
                  }}</span>
                </div>
              </div>
            </div>
      ...
      <span
        class="iconfont icon-09"
        v-if="socket && !isConnect"
        title="断线重连"
      ></span>
    </div>
    <div class="app-main-touch" v-if="loginUser.id && deviceType === 'phone'">
      ...
      <SessionPanel
        ...
        @sendMessage="sendMessage"
        :session="curSession"
        v-show="curSession.id"
      >
        <template slot="num"> ({{ users.length }}) </template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      about: {
        ...
        license: "MIT",
        author: "jacksonyang",
        email: "1422315999@qq.com",
        github: "https://github.com/while999/jacksonyang.git",
      },
      loginUser: {},
      token: "",
      deviceType: type,
  ...
  methods: {
    ...
    addSessionMessage(message, sessionId) {
      ...
      if (
        this.setting.isVoice &&
        this.curSession.id !== from.id &&
        from.id !== this.loginUser.id &&
        to.type === "user"
      ) {
        this.playAudio();
      }
